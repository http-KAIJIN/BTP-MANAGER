  ...
  return (
    <div className="p-6 lg:p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <Link href="/intervenants" className="text-sm text-orange-600 hover:underline">&larr; Back to Intervenants</Link>
          <h1 className="mt-1 text-3xl font-bold tracking-tight text-slate-950">{intervenant.name}</h1>
        </div>
        <div className="flex gap-3">
          <Link href={`/intervenants/${id}/edit`} className="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-medium hover:bg-slate-50">Edit</Link>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="text-lg font-bold text-slate-900">Intervenant Information</h2>
            <dl className="mt-4 grid grid-cols-2 gap-4 text-sm">
              <div><dt className="text-slate-500">Name</dt><dd className="font-medium text-slate-900">{intervenant.name}</dd></div>
              <div><dt className="text-slate-500">Phone</dt><dd className="font-medium text-slate-900">{intervenant.phone || '-'}</dd></div>
              <div><dt className="text-slate-500">Trade</dt><dd className="font-medium text-slate-900">{intervenant.trade}</dd></div>
              <div><dt className="text-slate-500">Status</dt><dd><span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${intervenant.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>{intervenant.status}</span></dd></div>
              <div className="col-span-2"><dt className="text-slate-500">Notes</dt><dd className="font-medium text-slate-900">{intervenant.notes || '-'}</dd></div>
            </dl>
          </div>
        </div>
      ...
      <DeleteModal
        ...
        loading={deleting}
        message="Are you sure you want to delete this intervenant? This action cannot be undone."
      />
    </div>
  );
}
