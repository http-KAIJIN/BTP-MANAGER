...
export default function NewProperty() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!form.reference.trim() || !form.surface || !form.price || !form.projectId) { setError(dict.errors.validation); return; }
    setSaving(true);
    setError("");
    try {
      await api.post("/real-estate/properties", { ...form, surface: Number(form.surface), price: Number(form.price) });
    ...
    <div className="mx-auto max-w-3xl space-y-6 p-4 sm:p-6 lg:p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <FormSection title={dict.properties.detail}>
          ...
          <SelectField label={dict.properties.project} value={form.projectId} onChange={(v) => update("projectId", v)} options={projects.map((p) => ({ value: p.id, label: `${p.name} - ${p.city}` }))} required full />
